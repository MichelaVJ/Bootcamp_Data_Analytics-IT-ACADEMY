...
WHERE EXISTS
	...
	AND t.amount > (SELECT avg(amount) as avg_total_sales FROM transaction));
...
SELECT DATE(timestamp) AS transaction_date, COUNT(*) AS num_transactions, SUM(amount) AS tot_sales
FROM transaction
...
SELECT company_name, country, t.id, timestamp 
FROM transaction t
